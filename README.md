...
- Library API: `selectors::watch::watch_commands` returns a `WatchHandle` (`events`, `report`) instead of a tuple, and `WatchError` is `#[non_exhaustive]` and has `NothingWatched` instead of `Io`.
